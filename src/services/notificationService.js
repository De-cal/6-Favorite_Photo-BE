...
const notificationService = {
  ...
  createExchangeSuccessNotification: async (buyerUserId, counterpartNickname, rank, title) => {
  ...
  createExchangeFailureNotification: async (buyerUserId, counterpartNickname, rank, title) => {
    const message = `${counterpartNickname} 님과의 [${rank} | ${title}] 포토카드 교환이 불발되었습니다.`;
    await notificationRepository.createNotification(message, [buyerUserId]);
  },

  // ?? 포토 카드 구매 성사 알림 생성 (구매자 대상)
  createPurchaseSuccessNotification: async (buyerUserId, rank, title, quantity) => {
    const message = `[${rank} | ${title}] ${quantity}장을 성공적으로 구매했습니다.`;
    await notificationRepository.createNotification(message, [buyerUserId]);
  },

  // 포토 카드 교환 제안 알림 생성 (판매자 대상)
  createExchangeOfferNotification: async (sellerUserId, proposingUserNickname, rank, title) => {
  ...
  createSaleSuccessNotification: async (sellerUserId, rank, title, quantity) => {
